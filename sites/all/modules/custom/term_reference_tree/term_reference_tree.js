...
Drupal.behaviors.termReferenceTree = {
  ...
  $('li :checkbox, li :radio').on('click', function () {
  
  var $cur_tab = $('li.selected',window.parent.document).find("strong").text();
  
 /* var $rfp_text = jQuery('#edit-field-rfp-product-und-0-field-categories').find('label').text();
  $rfp_cat = $rfp_text.slice(0, $rfp_text.indexOf(" *")); */
	//console.log($rfp_cat); 
    ...
    var $chk = $(this),
	   ...
	   if ($li.has('ul')) {
			$li.find(':checkbox').not(this).prop('checked', false)
			$li.find(':radio').not(this).prop('checked', false)
		}

	
	//alert(len);
	
    var $parents = $ul.parents('li').not('#term-parents-list');
   
  
   // console.log($parents);
    var count = $ul.has(':checkbox:checked').length;
	
	

	//console.log($current_super_parents);
	
	//var $radio_parents = $ul.closest('ul');
	// var radio_count = $ul.has(':radio:checked').length;
	 
	 //$radio_parents.find('input[type=radio]').prop('checked', false);	
	//var $all_radio = 
	//console.log(radio_count);
	
	/*	$radio_parents.each(function(key, obj) {
			 console.log($(obj).find('li'));
			 $(obj).find('li').find('input[type=radio]').prop('checked', false);	
			// $(obj).closest('ul#term-parents').closest('input[type=radio]').prop('checked', false);
			/*var obj_id  = $(obj).attr("id");
			var current_parent_id = $current_super_parents.attr("id");			
			if (obj_id !== current_parent_id) {				 
	...
	// });
	/**  add this code after RFP is deployed in production  -  || $rfp_cat == 'Product Category' **/
	
	if($cur_tab == 'Categories'){
		var $super_parents = $('li.term-super-parents-list');	
		var $current_super_parents = $ul.closest('li.term-super-parents-list');
		$super_parents.each(function(key, obj) {
				// console.log($(obj));
				var obj_id  = $(obj).attr("id");
				var current_parent_id = $current_super_parents.attr("id");			
				if (obj_id !== current_parent_id) {				 
	...
	//var flag;
    $parents.each(function(key, obj) {
	// console.log(obj);
	
		
      if (!isChecked && count > 0) {
	   //alert($(obj).children().not('ul').has(":checkbox:checked").length);
	  		
        $(obj).children().not('ul').find(":checkbox").prop('checked', true);
        $(obj).children().not('ul').find(":radio").prop('checked', true);
		
      }
      else {
			$(obj).children().not('ul').find(":checkbox").prop('checked', true);
			$(obj).children().not('ul').find(":radio").prop('checked', true);
       //  $(obj).children().not('ul').find(":checkbox").prop('checked', isChecked);
       //  $(obj).children().not('ul').find(":radio").prop('checked', isChecked);
	...
	//alert(flag);
    if (!isChecked && count > 0){
     $ul.parents("li#term-parents-list").children('div.form-type-radio').find(":radio").prop('checked', true);
	
    }else {
	
     $ul.parents("li#term-parents-list").children('div.form-type-radio').find(":radio").prop('checked', isChecked);
	} 
